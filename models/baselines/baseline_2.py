...
def training_epoch(
    ...
    epoch_accuracy, epoch_reward, loss, model = epoch_loss(
        epoch,
        args,
        optimizer,
        device,
...
def validation_epoch(
    ...
    with torch.no_grad():
        ...
        if validation_reward_mean > best_validation_reward:
            ...
            torch.save(
                model.state_dict(),
                "./saved_models/" + args.classifier_model + "_baseline_2_best.pt",
            )
...
def epoch_loss(
    ...
    returns:
        ...
        epoch_reward: the average epoch reward, which computed based only on accuracy.
        loss: the  epoch loss
        model: the pretrained classifier
    """
    ...
    for i in range(int(np.ceil(len(data) / args.batch_size))):
        ...
        rewards_acc, rewards_total, accuracy = [], [], []
        ...
        rewards_total.append(torch.tensor(args.lambda_PG * reward_acc))
    ...
    return epoch_accuracy, epoch_reward, loss, model


def train_baseline_2(args):
    """
    Train the model present in https://arxiv.org/pdf/1911.03861.pdf by passing over the training and validation data to fine-tune the pretrained model on the minority examples only.
    ...
    test_data = pd.read_csv("./data/" + args.dataset + "_test_original_gender.csv")
    ...
    for epoch in range(args.num_epochs_PG):
        training_loss, model = training_epoch(
            ...
            tokenizer,
            model,
            train_data[train_data["minority"] == True],
            train_data_gender_swap[train_data_gender_swap["minority"] == True],
        )
